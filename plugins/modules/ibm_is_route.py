...
---
module: ibm_is_route
short_description: Manage IBM Cloud Route
version_added: "1.0.0"
description:
    - Create, update, or delete IBM Cloud Route
...
class IBMRouteModule(IBMCloudSDKModule):
    """IBM Cloud Route module implementation."""
    ...
    def get_resource(self, resource_id: str):
        """Get resource by ID."""
        try:
            response = self.vpc_service.get_vpc_routing_table_route(id=resource_id)
    ...
    def create_resource(self):
        ...
        try:
            ...
            'routing_table': self.params.get('routing_table'),
            'destination': self.params.get('destination'),
            'zone': self.params.get('zone'),
            'next_hop': self.params.get('next_hop')
        }
            
            response = self.vpc_service.create_vpc_routing_table_route(**prototype)
            resource = response.get_result()
            
            self.result['changed'] = True
            self.result['resource'] = resource
            self.result['msg'] = f"route {self.resource_name} created successfully"
    ...
    def update_resource(self, resource):
        ...
        self.result['resource'] = resource
        self.result['msg'] = f"route {resource['name']} " + ("updated" if changed else "unchanged")
    ...
    def delete_resource(self, resource_id: str):
        ...
        try:
            self.vpc_service.delete_vpc_routing_table_route(id=resource_id)
            self.result['changed'] = True
