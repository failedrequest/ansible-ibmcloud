...
DOCUMENTATION = r'''
---
module: ibm_tg_connection_prefix_filter
short_description: Manage IBM Cloud Tg Connection Prefix Filter
version_added: "1.0.0"
description:
    - Create, update, or delete IBM Cloud Tg Connection Prefix Filter
...
EXAMPLES = r'''
- name: Create Tg Connection Prefix Filter
  ibm_tg_connection_prefix_filter:
    name: my-tg-connection-prefix-filter
    state: present

- name: Delete Tg Connection Prefix Filter
  ibm_tg_connection_prefix_filter:
    id: resource-id-123
    state: absent
...
RETURN = r'''
resource:
    description: Tg Connection Prefix Filter information
    returned: always
    type: dict
changed:
...
class IBMTgConnectionPrefixFilterModule(IBMCloudSDKModule):
    ...
    def get_resource(self, resource_id: str):
        """Get resource by ID."""
        try:
            response = self.service.get_transit_gateway_connection_prefix_filter(id=resource_id)
    ...
    def list_resources(self):
        """List all resources."""
        try:
            response = self.service.list_transit_gateway_connection_prefix_filters()
