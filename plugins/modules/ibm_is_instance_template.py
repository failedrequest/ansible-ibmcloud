...
short_description: Manage IBM Cloud Instance Template
version_added: "1.0.0"
description:
    - Create, update, or delete IBM Cloud Instance Template
...
class IBMInstanceTemplateModule(IBMCloudSDKModule):
    ...
    def get_resource(self, resource_id: str):
        """Get resource by ID."""
        try:
            response = self.vpc_service.get_instance_template(id=resource_id)
        ...
        if updates:
            ...
            try:
                response = self.vpc_service.update_instance_template(
                    id=resource['id'],
                    instance_template_patch=updates
                )
                resource = response.get_result()
                changed = True
            except ApiException as e:
...
def main():
    ...
    argument_spec.update({
        ...
        'primary_network_interface': {'type': 'str', 'required': False},
