...
DOCUMENTATION = r'''
---
module: ibm_en_subscription
short_description: Manage IBM Cloud En Subscription
version_added: "1.0.0"
description:
    - Create, update, or delete IBM Cloud En Subscription
...
EXAMPLES = r'''
- name: Create En Subscription
  ibm_en_subscription:
    name: my-en_subscription
    state: present

- name: Delete En Subscription
  ibm_en_subscription:
    id: resource-id-123
    state: absent
...
RETURN = r'''
resource:
    description: En Subscription information
    returned: always
    type: dict
changed:
...
class IBMEnSubscriptionModule(IBMCloudSDKModule):
    ...
    def get_resource(self, resource_id: str):
        """Get resource by ID."""
        try:
            response = self.service.get_subscription(id=resource_id)
