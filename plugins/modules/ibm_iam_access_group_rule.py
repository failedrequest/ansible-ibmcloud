...
short_description: Manage IBM Cloud Iam Access Group Rule
version_added: "1.0.0"
description:
    - Create, update, or delete IBM Cloud Iam Access Group Rule
...
EXAMPLES = r'''
- name: Create Iam Access Group Rule
  ibm_iam_access_group_rule:
    name: my-iam_access_group_rule
    state: present

- name: Delete Iam Access Group Rule
  ibm_iam_access_group_rule:
    id: resource-id-123
    state: absent
...
RETURN = r'''
resource:
    description: Iam Access Group Rule information
    returned: always
    type: dict
changed:
...
class IBMIamAccessGroupRuleModule(IBMCloudSDKModule):
    ...
    def get_resource(self, resource_id: str):
        """Get resource by ID."""
        try:
            response = self.service.get_access_group_rule(id=resource_id)
